...
class ImageGraph:
    ...
    def create_adjacency_matrix(self):
        ...
        for _ in range(len(self.vertices)):
            adj_matrix.append([0] * len(self.vertices))

        for i in range(len(self.vertices)):
            for j in range(len(self.vertices[i].edges)):
                adj_matrix[i][self.vertices[i].edges[j]] = 1
    ...
    def bfs(self, start_index, color):
        ...
        start_vert = self.vertices[start_index]
        queue = Queue()
        queue.enqueue(start_vert)
        start_vert.visit_and_set_color(color)
...
def create_graph(data):
    ...
    image_size = int(info[0])
    vert_num = int(info[1])
    # create the ImageGraph
    graph = ImageGraph(image_size)
    # create vertices - vertex info has the format "x,y,color"
    vert_list = info[2:vert_num + 2]
    for i in range(len(vert_list)):
        vertex_info = vert_list[i].split(",")
        vertex = ColoredVertex(i, int(vertex_info[0]), int(vertex_info[1]), vertex_info[2])
    ...
    edge_num = int(info[vert_num + 2])
    edge_list = info[vert_num + 3:vert_num + 3 + edge_num]
    for i in range(len(edge_list)):
        edge_info = edge_list[i].split(",")
        graph.vertices[int(edge_info[0])].add_edge(int(edge_info[1]))
        graph.vertices[int(edge_info[1])].add_edge(int(edge_info[0]))
    # read search starting position and color
    start_info = info[-2].split(",")
    start_pos = int(start_info[0])
    fill_color = start_info[1]
    # return the ImageGraph, starting position, and color as a tuple in this order.
    return (graph, start_pos, fill_color)
...
def main():
    ...
    adj_list = image_graph.create_adjacency_matrix()
    for row in adj_list:
        print(row)
    # run bfs
    image_graph.bfs(start_pos, fill_color)
    ...
    fill_color = graph[2]
    # run dfs
    image_graph.dfs(start_pos, fill_color)
